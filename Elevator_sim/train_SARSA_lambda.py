...
# alpha = 0.1
# lambd = 0.9
# # Function to update the Q-value using TD(λ)
# def update_q_value(q_value, q_target, eligibility_trace, alpha, gamma, lambd):
#     delta = q_target - q_value
#     eligibility_trace *= lambd * gamma  # Decay eligibility trace
#     eligibility_trace += 1.0
#     q_value += alpha * delta * eligibility_trace
#     return q_value
...
#         q_value = q_network(torch.tensor(state, dtype=torch.float32))
#         Q_s_a = q_value[action]
#         eligibility_trace[action] += 1.0  # Increment eligibility trace for the chosen action
#         q_value = update_q_value(Q_s_a, q_target, eligibility_trace[action], alpha, gamma, lambd)
